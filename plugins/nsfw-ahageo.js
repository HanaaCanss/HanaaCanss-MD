//CASE BUATAN HANA KAKKK , JANGAN DIHAPUSS YAHH SEMUA SEMUA DI HAPUSS :') CAPE TAUU.
let fetch = require('node-fetch')
let handler = async (m, { conn }) => {
	let url = ahageo[Math.floor(Math.random() * ahageo.length)]
	conn.sendButtonImg(m.chat, url, 'TcihhhDasarr', wm2, 'Next', '.ahageo', m)
}
handler.help = ['ahageo']
handler.tags = ['nsfw']
handler.command = /^(ahageo)$/i
